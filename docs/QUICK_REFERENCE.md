...
# Or explicit per-type map (default shown):
web_urls:
  citations: rich              # off | minimal | rich
  conv_safe_urls: minimal      # off | minimal
  msg_safe_urls: off           # off | minimal
  search_result_groups: off    # off | minimal | rich
  content_references: off      # off | minimal | rich
```
